...
/**
 * 채팅방 -> 채팅방응답 DTO로 바꿔주는 컨버터
 * - targetUserId와 myUnreadCount는 '현재 사용자' 관점으로 계산한다.
 *
 */
@Component
public class ChatConverter {
   ...
   *
   * "현재 사용자(userId)" 관점으로 아래 두 값을 치환해서 내려준다.
   *  - targetUserId   : 상대방 사용자 ID
   *  - myUnreadCount : '내' 안읽음 카운트
   *
   * 예)
   *  room(userAId=3, userBId=10, unreadA=0, unreadB=5)
   *   - userId=10 → targetUserId=3, myUnreadCount=5
   *   - userId=3  → targetUserId=10, myUnreadCount=0
   *
   * @param room 채팅방 엔티티
